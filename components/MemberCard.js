...
const MemberCard = ({ name, role, bio, isMain }) => {
  ...
  return (
    <>
      <div
        className={`bg-white shadow-lg rounded-lg overflow-hidden transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl ${
          isMain ? 'col-span-3 row-span-2' : ''
        }`}
      >
        ...
        <div className="p-4">
          <h3 className={`font-semibold text-gray-800 ${isMain ? 'text-2xl' : 'text-xl'}`}>{name}</h3>
